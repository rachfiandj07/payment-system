...
export class PaymentService {
  ...
  public async generate(
    ...
    try {
      const data: any = await this.invoiceService.findByID(
        createPaymentLinkDTO.invoices_id,
      );
      if (!data) {
        return {
      ...
      const base_url =
        context.baseUrl + '/' + this.configService.getOrThrow('PAYMENT_PREFIX');
      ...
      /* 
            ...
            */
      const linkPromise = this.generatePaymentLink(
        data.customers_id,
        data.amount_due,
        expirationTime,
        base_url,
