...
export class InvoiceService {
  ...
  public async create(
    ...
    try {
      if (createInvoiceDTO.customers_id !== undefined) {
        const data: any = await this.customerService.findByID(
          createInvoiceDTO.customers_id,
        );
        ...
        const invoiceData = await this.databaseService.invoices.create({
          data: invoice,
        });
        ...
        const data: any = await this.customerService.create(
          createInvoiceDTO.customers,
        );
        /*
                        invoice_date: Date;
                        amount_due: string;
        ...
        createInvoiceDTO.customers_id === undefined &&
        createInvoiceDTO.customers_id === undefined
      ) {
        return {
          statusCode: HttpStatus.NOT_FOUND,
  ...
  private generateInvoiceID(customer_id: string): string {
    ...
    const invoiceId =
      `INV${customer_id}${year}${month}${day}${hours}${minutes}${seconds}`
        .toUpperCase()
        .replace(/-/g, '');
